...
def add(sector_name):
    ...
    :return: JSON string containing data about the sector
    """
    try:
        JSONValidator.check_parameters(object=Sector, json_dict=request.json)
...
def get(name):
    """
    Given the name of a sector, queries the database
    :param name: The name of the sector
    :return: JSON string containing data about the sector, if found. Else, a JSON string
    with an error message
    """
    # We try to find the sector
    sector = Sector.query.filter_by(name=name).first()

    # If the sector does not exist
    if not sector:
...
def update(sector_name):
    ...
    try:
        JSONValidator.check_parameters(object=Sector, json_dict=request_json)
    except NotEnoughParametersException as e:
        return e.response_json, 422
    except InvalidParameterTypeException as e:
        return e.response_json, 422
